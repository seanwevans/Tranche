...
const getCustomerIDForToken = `-- name: GetCustomerIDForToken :one
SELECT customer_id
...
func (q *Queries) GetCustomerIDForToken(ctx context.Context, tokenHash string) (int64, error) {
	row := q.db.QueryRowContext(ctx, getCustomerIDForToken, tokenHash)
